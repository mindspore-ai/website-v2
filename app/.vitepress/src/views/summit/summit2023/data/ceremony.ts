import dingcheng from '../img/guest/dingcheng.png';
import huxiaoman from '../img/guest/huxiaoman.png';
import songhaitao from '../img/guest/songhaitao.png';
import wangjinqiao from '../img/guest/wangjinqiao.png';
import xulingjie from '../img/guest/xulingjie.png';
import yanghua from '../img/guest/yanghua.png';
import tianyonghong from '../img/guest/tianyonghong.png';
import huangxiangjun from '../img/guest/huangxiangjun.png';
import wangbin from '../img/guest/wangbin.png';
import suliqiang from '../img/guest/suliqiang.png';
import liuming from '../img/guest/liuming.png';
import jinxuefeng from '../img/guest/jinxuefeng.png';
import suteng from '../img/guest/suteng.png';
import yangyi from '../img/guest/yangyi.png';
import yufan from '../img/guest/yufan.png';
import yuyue from '../img/guest/yuyue.png';
import chenlei from '../img/guest/chenlei.png';
import dongbin from '../img/guest/dongbin.png';
import sunxian from '../img/guest/sunxian.png';

import wulingxiang from '../img/guest/wulingxiang.png';
import mailuo from '../img/guest/mailuo.png';
import linyonghua from '../img/guest/linyonghua.png';

import chenmingming from '../img/guest/chenmingming.png';
import xieyuan from '../img/guest/xieyuan.png';
import yujun from '../img/guest/yujun.png';
import douzhicheng from '../img/guest/douzhicheng.png';
import xielei from '../img/guest/xielei.png';
import yanyonghong from '../img/guest/yanyonghong.png';
import jiangli from '../img/guest/jiangli.png';

export default [
  {
    title: '昇思MindSpore开源社区理事会成立仪式嘉宾',
    content: [
      {
        img: dingcheng,
        name: '丁诚',
        position: ['昇思MindSpore开源社区理事长'],
      },
      {
        img: huxiaoman,
        name: '胡晓曼',
        position: ['昇思MindSpore开源生态总监'],
      },
      {
        img: huangxiangjun,
        name: '黄向军 ',
        position: ['沐曦研发副总裁'],
      },
      {
        img: liuming,
        name: '刘明',
        position: ['大华股份高级副总裁'],
      },
      {
        img: songhaitao,
        name: '宋海涛 ',
        position: ['上海人工智能研究院执行院长','上海交通大学人工智能研究院副院长'],
      },
      {
        img: suliqiang,
        name: '苏力强 ',
        position: ['博瀚智能首席技术官'],
      },
      {
        img: tianyonghong,
        name: '⽥永鸿',
        position: [
          '鹏城实验室⽹络智能研究部副主任',
          '北京⼤学深圳研究⽣院信息⼯程学院院⻓',
        ],
      },
      {
        img: wangbin,
        name: '王斌 ',
        position: ['达闼机器人有限公司产品副总裁'],
      },
      {
        img: wangjinqiao,
        name: '王⾦桥',
        position: [
          '中国科学院⾃动化研究所',
          '紫东太初⼤模型研究中⼼常务副主任',
        ],
      },
      {
        img: xulingjie,
        name: '徐凌杰 ',
        position: ['壁仞科技','联合创始人、总裁'],
      },
      {
        img: yanghua,
        name: '杨华 ',
        position: ['中科弘云CEO'],
      },
    ],
  },
  {
    title: '昇思MindSpore开源社区技术委员会专家聘请仪式嘉宾',
    content: [
      {
        img: jinxuefeng,
        name: '⾦雪锋',
        position: ['昇思MindSpore总架构师'],
      },
      {
        img: yufan,
        name: '于璠 ',
        position: ['昇思MindSpore首席科学家'],
      },
      {
        img: suteng,
        name: '苏腾',
        position: ['昇思MindSpore架构师'],
      },
      {
        img: chenlei,
        name: '陈雷',
        position: ['昇思MindSpore首席科学家', '加拿大滑铁卢大学计算机系博士'],
      },
      {
        img: yangyi,
        name: '杨奕',
        position: ['深圳湾实验室副研究员'],
      },
      {
        img: dongbin,
        name: '董彬',
        position: [
          '北京大学国际数学研究中心教授',
          '国际机器学习研究中心副主任',
        ],
      },
      {
        img: sunxian,
        name: '孙显',
        position: [
          '中国科学院空天信息创新研究院研究员',
          '博导 中科院重点实验室副主任',
        ],
      },
      {
        img: wulingxiang,
        name: '吴凌翔',
        position: ['武汉人工智能研究院 ', '算法总监'],
      },
      {
        img: jiangli,
        name: '蒋力',
        position: ['上海交通大学计算机科学与工程系', '副教授 博导'],
      },
      {
        img: mailuo,
        name: '麦络',
        position: ['爱丁堡大学助理教授'],
      },
      {
        img: yujun,
        name: '於俊',
        position: ['中国科学技术大学', '副教授'],
      },
      {
        img: xieyuan,
        name: '谢源',
        position: ['华东师范大学计算机科学与技术学院', '教授 博导'],
      },
      {
        img: chenmingming,
        name: '程明明',
        position: ['南开大学杰出教授', '计算机系主任'],
      },
      {
        img: douzhicheng,
        name: '窦志成',
        position: ['中国人民大学高瓴人工智能学院副院长', '教授 博导'],
      },
      {
        img: xielei,
        name: '谢磊',
        position: ['西北工业大学计算机学院院长助理', '教授 博导'],
      },
      {
        img: yanyonghong,
        name: '颜永红',
        position: ['中科院声学所首席科学家'],
      },
      {
        img: yuyue,
        name: '余跃',
        position: ['OpenI启智开源平台运营中心主任','AITISA联盟算力网推进组长'],
      },
      {
        img: linyonghua,
        name: '林咏华',
        position: ['智源研究院总工程师','IEEE女工程师亚太区领导组成员'],
      },
    ],
  },
];
